...
```{r}
#why not add DOP?
zoop_data<-Zoopsynther(Data_type="Community", Sources=c("EMP", "STN", "20mm", "FMWT", "DOP"), Time_consistency = FALSE)

sum = group_by(zoop_data, Taxlifestage) %>%
  summarize(min = min(CPUE), max = max(CPUE), median = median(CPUE))
...
```{r}
zoop_data_mass<-mutate(zoop_data_mass, Subregion_edit = case_match(SUBREGION, c("NE Suisun", "SE Suisun", "NW Suisun", 
                                                                  "SW Suisun")~"Suisun Bay", 
                                                                  c("Confluence", "Lower Sacramento River") ~ "Lower Sacramento",.default = SUBREGION))
...
sal_model<-function(group,region,new_data=newdata){
  ...
  if(length(unique(data$Station_fac))>1){
    model<-bam(BPUE_log1p ~ te(SalSurf_s, doy_s, k=c(5,5), bs=c("cs", "cs")) + 
...
sal_conversionsa<-pmap_dfr(model_factors[c(1:10),], function(IBMR, Subregion_edit) sal_model(IBMR, Subregion_edit), .id = "IBMR_region")

sal_conversionsb<-pmap_dfr(model_factors[c(11:20),], function(IBMR, Subregion_edit) sal_model(IBMR, Subregion_edit), .id = "IBMR_region")

sal_conversionsc<-pmap_dfr(model_factors[c(21:30),], function(IBMR, Subregion_edit) sal_model(IBMR, Subregion_edit), .id = "IBMR_region")

sal_conversionsd<-pmap_dfr(model_factors[c(31:39),], function(IBMR, Subregion_edit) sal_model(IBMR, Subregion_edit), .id = "IBMR_region")

#turn it into a data fame and make it more useful. 
sal_conversions = bind_rows(sal_conversionsa, sal_conversionsb, sal_conversionsc, sal_conversionsd)%>%
  mutate(IBMR=sapply(IBMR_region, function(x) str_split(x, " ", n=2)[[1]][1]),
         Region=factor(sapply(IBMR_region, function(x) str_split(x, " ", n=2)[[1]][2]),
                       levels=c("Lower Sacramento", "Suisun Marsh", "Suisun Bay")),
         Month=as.integer(Month))%>%
  select(-IBMR_region)%>%
  relocate(Region, Month, IBMR, SalSurf)

#save the results
save(sal_conversions,  file = "outputs/salinityConversions_threeregions.RData")
...
ggplot(pseudoSuisun, aes(x = Scenario, y = MeanBiomass, fill = Scenario, group = Year))+ geom_col(position = "dodge", color = "grey", aes(alpha = Year))+
  ...
  ylab("Mean summer Pseudodiaptomus \nbiomass (ug/m3)")

ggsave("plots/SummerPseudoBiomass.png", device = "png", width =8, height =6)
...
zoop_sal2024<-dat2020%>%
  distinct() %>%
  mutate(Subregion_edit=case_match(region, c("NE Suisun", "SE Suisun", "SW Suisun", "NW Suisun")~"Suisun Bay",
                                   c("Lower Sacramento River", "Confluence")~"Lower Sacramento",
                                   
                                  .default = as.character(region))) %>%
mutate(Salinity=as.character(mean), Year = 2024,  DOY = yday(time))%>%
  left_join(distinct(model_factors)) %>%
  ungroup()
...
write.csv(step424, here("Outputs", "zoop sal adjustments_2024gates_c.csv"), row.names = F)

step424 = read_csv("Outputs/zoop sal adjustments_2024gates_c.csv")
...
zoopscon24 = left_join(zoopsmedian2024b, zoopsU952024b)%>%
  ...
  summarize(Biomass = mean(Biomass, na.rm =T), BiomassU95 = mean(BiomassU95, na.rm =T),
         ...
         mutate(Biomass = case_when(Taxon == "Limnoithona" ~ Biomass/2,
                             TRUE ~ Biomass))%>%
  mutate(Bigregion = case_when(Bigregion == "Confluence" ~ "Lower Sacramento",
                               TRUE ~ Bigregion))
...
zoopscon24 = mutate(zoopscon24, Scenario2 = factor(Scenario, levels = c("NoAction", "GatesOnly", "X2Only"),
                                                   labels = c("No-Action", "SMSCG, No X2", "No SMSCG, X2")))
ggplot(filter(zoopscon24,Scenario != "Gates+X2"), aes(x = DOY, y = Biomass, fill = Taxon))+
  geom_area()+
  facet_grid(Bigregion~Scenario2, scales = "free_y")+
  
  scale_fill_manual(values = mypal)+
  scale_x_continuous(breaks = c(183, 214, 245, 275), labels = c("Jul", "Aug", "Sep", "Oct"), limits = c(180, 305))+
  xlab("Day of Year")+ theme_bw()

ggplot(filter(zoopscon24b, Taxon == "Pseudodiaptomus", !is.na(Month), Month !=6), aes(x = Bigregion, y = Biomass, fill = Scenario))+
...
ggplot(filter(zoopscon24b, Taxon == "Pseudodiaptomus", !is.na(Month), Scenario != "Gates+X2", Month !=6), aes(x = Month, y = Biomass, fill = Scenario))+
  scale_fill_manual(values = c("grey40", "dodgerblue3", "red3"), labels = c("No-Action", "No SMSCG, X2", "SMSCG, No X2"))+
...
ggplot(limnoplot, aes(x = SalSurf, y = median)) + geom_line()+
  ...
  geom_ribbon(aes(ymin = l95, ymax = u95))+scale_y_log10()
```

```{r}
limno2 = filter(zoop_data_mass, IBMR == "limno", Subregion_edit %in% c("Suisun Bay", "Suisun Marsh"))
...
library(readxl)
library(wql)
juneEC = read_excel("data/EC_Graph_052725.xlsx", sheet = "Beldons Landing") %>%
  select(AA, `Observed EC`, `SMSCG Open`, `SMSCG Tidal Op`)%>%
  pivot_longer(cols = c(`Observed EC`, `SMSCG Open`, `SMSCG Tidal Op`),
               names_to = "Scenario", values_to = "EC")%>%
  mutate(Salinity = ec2pss(EC/1000, 25)) %>%
  filter(!is.na(AA))
...
#first take the salinity data and expand it to include all IBMR groups.
zoop_june<-juneEC%>%
  mutate(Subregion_edit="Suisun Marsh") %>%
mutate(Salinity=as.character(round(Salinity, 1)), Year = 2025,  DOY = yday(AA))%>%
  left_join(distinct(model_factors)) %>%
  ungroup()
...
#calculate the median and 95% confidence interval across simulations.
step4june = step3%>%
  mutate(across(c(`Observed EC`:last_col()), ~.x/as.numeric(Salinity)))%>%
  group_by(Subregion_edit, Year, DOY, IBMR)%>%
  summarise(across(c(`Observed EC`:last_col()), 
                   list(median=~median(.x, na.rm=T), 
                        l95=~quantile(.x, 0.025, na.rm=T), 
                        u95=~quantile(.x, 0.975, na.rm=T))), 
            .groups="drop")
...
write.csv(step4june, here("Outputs", "zoop sal adjustments_2025june.csv"), row.names = F)

step4june = read_csv("Outputs/zoop sal adjustments_2025june.csv")

zoopsmedian2024june = pivot_longer(step4june,  cols = c(`Observed EC_u95`:last_col()),
       names_to = "Scenario", values_to = "Biomass") %>%
filter(!str_detect(Scenario, "95")) %>%
  mutate(Scenario = str_remove(Scenario, "_median"))


zoopsupper2024june = pivot_longer(step4june,  cols = c(`Observed EC_u95`:last_col()),
       names_to = "Scenario", values_to = "Biomass_U95") %>%
filter(str_detect(Scenario, "_u95"))%>%
  mutate(Scenario = str_remove(Scenario, "_u95"))


zoopslower2024june = pivot_longer(step4june,  cols = c(`Observed EC_u95`:last_col()),
       names_to = "Scenario", values_to = "Biomass_L95") %>%
filter(str_detect(Scenario, "l95")) %>%
  mutate(Scenario = str_remove(Scenario, "_l95"))

alljunepseudo = left_join(zoopsmedian2024june, zoopslower2024june) %>% left_join(zoopsupper2024june) %>%
  filter(IBMR == "pdiapjuv") %>%
  group_by(Scenario) %>%
  summarize(Biomass = mean(Biomass, na.rm =T), Biomass_L95 = mean(Biomass_L95, na.rm =T), Biomass_U95 = mean(Biomass_U95, na.rm =T))
  

ggplot(zoopsmedian2024june, aes(x = DOY, y = Biomass, fill = IBMR))+ geom_area()+
  facet_wrap(~Scenario)+
  xlab("Day of Year (2025)")


#just pseudodiaptomus
ggplot(alljunepseudo, aes(x = Scenario, y = Biomass))+ geom_col()+
  geom_errorbar(aes(ymin = Biomass_L95, ymax = Biomass_U95))


```

Graph of both June and September for 2-pager

```{r}
september = filter(zoopscon24, Taxon == "Pseudodiaptomus", region == "Suisun Marsh", Month == 9,
                   Scenario %in% c("NoAction", "GatesOnly")) %>%
  rename(Biomass_L95= lower, Biomass_U95 = upper) %>%
  mutate(Month = "September", Scenario = case_match(as.character(Scenario), "NoAction" ~ "SMSCG Open",
                                                    "GatesOnly" ~ "SMSCG Tidal Op"))

suisunX = filter(zoopscon24, Taxon == "Pseudodiaptomus", region == "Suisun Marsh", 
                   Scenario %in% c("NoAction", "GatesOnly"), Month !=6) %>%
  rename(Biomass_L95= lower, Biomass_U95 = upper) %>%
  mutate(Scenario = case_match(as.character(Scenario), "NoAction" ~ "SMSCG Open",
                                                    "GatesOnly" ~ "SMSCG Tidal Op"))

#just september and june
pseudo2024 = bind_rows(mutate(alljunepseudo, Month = "June"), september)


ggplot(pseudo2024, aes(x = Scenario, y = Biomass))+ facet_wrap(~Month)+
  geom_col()+
  geom_errorbar(aes(ymin = Biomass_L95, ymax = Biomass_U95))+
  ylab("Pseudodiaptomus Biomass (ug/m3)")

#all months

pseudo2024all = bind_rows(mutate(alljunepseudo, Month = 6), suisunX)

ggplot(pseudo2024all, aes(x = Month, y = Biomass, fill = Scenario, group = Scenario))+ 
  geom_col(position = "dodge")+
  geom_errorbar(aes(ymin = Biomass_L95, ymax = Biomass_U95), position = "dodge")+
  ylab("Pseudodiaptomus Biomass (ug/m3)")
