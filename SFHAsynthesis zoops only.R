...
Wt.summary_constanttemp = all.Wt2_constanttemp %>%
...
Wt.summary_constanttemp = left_join(Wt.summary_constanttemp, sal) %>%
  mutate(MWeight6 = case_when(GoodSalinity ~ MWeight,
                              TRUE ~ 0),
         MWeight2 = case_when(GoodSalinity2 ~ MWeight,
                              TRUE ~ 0))

ggplot(Wt.summary_constanttemp, aes(x = Day, y = MWeight, color = Region2)) +
  geom_smooth()+
  facet_wrap(~Year)

ggplot(Wt.summary_constanttemp, aes(x = Day, y = MWeight, color = as.factor(Year))) +
  geom_smooth()+
  facet_wrap(~Region)


Wt.sumx_constanttemp = left_join(Wt.summary_constanttemp, DF)

ggplot(Wt.sumx_constanttemp, aes(x = Day, y = MWeight, color = X2, group = as.factor(Year))) +
  geom_smooth()+
  facet_wrap(~Region2)+
...
Wtsum2_constanttemp = group_by(Wt.summary_constanttemp, Year, Region2) %>%
  summarize(startweight = first(MWeight), endwieght = last(MWeight), diffweight = endwieght-startweight)


ggplot(Wtsum2_constanttemp, aes(x = Region2, y = diffweight, fill = Region2)) + geom_col(position = "dodge")+
  facet_wrap(~Year, scales = "free_x")+
  theme(axis.text.x = element_blank())



Wtsum2_constanttemp = left_join(Wtsum2_constanttemp, DF)

ggplot(Wtsum2_constanttemp, aes(x = X2, y = diffweight)) + geom_point()+
...
#now we are seing the x2 effect, indicating zoopas are more important than temperature or  turbidity
#need to plot zoops in NW suisun versu X2, espeically for 2017. Maybe the limno explosion caused the wierd relationship?
#but NW suisun still has higher growth, even with temp held constant. Odd. 


Wt.summary_constanttemp = Wt.summary_constanttemp %>%
  group_by(Region2, Year) %>%
  mutate(WperD = MWeight - lag(MWeight),
         WperD = replace_na(WperD, 0)) %>%
  ungroup()

Meangrowth6_constanttemp = Wt.summary_constanttemp %>%
  group_by(Year, Day) %>%
  filter(GoodSalinity) %>%
...
Meangrowth2_constanttemp = Wt.summary_constanttemp %>%
  group_by(Year, Day) %>%
...
Meangrowth_constanttemp = left_join(Meangrowth6_constanttemp, Meangrowth2_constanttemp)

ggplot(Meangrowth_constanttemp, aes(x = Day, y = Weight,color = as.factor(Year))) + geom_line()+
  scale_color_manual(values = mypal)+ theme_bw()
ggplot(Meangrowth_constanttemp, aes(x = Day, y = Weight2,color = as.factor(Year))) + geom_line()

#Does this make sense? need to check with the group. 

Meangrowthsum_constanttemp = Meangrowth_constanttemp %>%
  group_by(Year) %>%
...
ggplot(Meangrowthsum_constanttemp, aes(x = X2, y = Weight)) + geom_point()+ geom_smooth(method = "lm")+
  geom_text(aes(label = Year))+ ggtitle("Only zoops change")+ ylab("Jun-October grown in regions <6 PSU")
#weight in 2022 wasw still really low. I need to get the zoops better organized. 

ggplot(Meangrowthsum_constanttemp, aes(x = X2, y = Weight)) + geom_point()+ geom_smooth()+
  geom_text(aes(label = Year))



#x2 versus growth 2 psu
ggplot(Meangrowthsum_constanttemp, aes(x = X2, y = Weight2)) + geom_point()+ geom_smooth(method = "lm")+
  geom_text(aes(label = Year))
