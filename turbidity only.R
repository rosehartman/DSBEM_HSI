#now have turbidity be the only thing that varies
...
Wt.summary_turbonly = all.Wt2_turbonly %>%
...
Wt.summary_turbonly = left_join(Wt.summary_turbonly, sal) %>%
  mutate(MWeight6 = case_when(GoodSalinity ~ MWeight,
                              TRUE ~ 0),
         MWeight2 = case_when(GoodSalinity2 ~ MWeight,
                              TRUE ~ 0))

ggplot(Wt.summary_turbonly, aes(x = Day, y = MWeight, color = Region2)) +
  geom_smooth()+
  facet_wrap(~Year)

ggplot(Wt.summary_turbonly, aes(x = Day, y = MWeight, color = as.factor(Year))) +
  geom_smooth()+
  facet_wrap(~Region)


Wt.sumx_turbonly = left_join(Wt.summary_turbonly, DF)

ggplot(Wt.sumx_turbonly, aes(x = Day, y = MWeight, color = X2, group = as.factor(Year))) +
  geom_smooth()+
  facet_wrap(~Region2)+
...
Wtsum2_turbonly = group_by(Wt.summary_turbonly, Year, Region2) %>%
  summarize(startweight = first(MWeight), endwieght = last(MWeight), diffweight = endwieght-startweight)


ggplot(Wtsum2_turbonly, aes(x = Region2, y = diffweight, fill = Region2)) + geom_col(position = "dodge")+
  facet_wrap(~Year, scales = "free_x")+
  theme(axis.text.x = element_blank())



Wtsum2_turbonly = left_join(Wtsum2_turbonly, DF)

ggplot(Wtsum2_turbonly, aes(x = X2, y = diffweight)) + geom_point()+
...
#now we are seing the x2 effect, indicating zoopas are more important than temperature
...
Wt.summary_turbonly = Wt.summary_turbonly %>%
  group_by(Region2, Year) %>%
  mutate(WperD = MWeight - lag(MWeight),
         WperD = replace_na(WperD, 0)) %>%
  ungroup()

Meangrowth6_turbonly = Wt.summary_turbonly %>%
  group_by(Year, Day) %>%
  filter(GoodSalinity) %>%
...
Meangrowth2_turbonly = Wt.summary_turbonly %>%
  group_by(Year, Day) %>%
...
Meangrowth_turbonly = left_join(Meangrowth6_turbonly, Meangrowth2_turbonly)

ggplot(Meangrowth_turbonly, aes(x = Day, y = Weight,color = as.factor(Year))) + geom_line()+
  scale_color_manual(values = mypal)+ theme_bw()
ggplot(Meangrowth_turbonly, aes(x = Day, y = Weight2,color = as.factor(Year))) + geom_line()

#Does this make sense? need to check with the group. 

Meangrowthsum_turbonly = Meangrowth_turbonly %>%
  group_by(Year) %>%
...
ggplot(Meangrowthsum_turbonly, aes(x = X2, y = Weight)) + geom_point()+ geom_smooth(method = "lm")+
  geom_text(aes(label = Year))+
  ggtitle("Only turbidity changes")
#weight in 2022 wasw still really low. I need to get the zoops better organized. 

ggplot(Meangrowthsum_turbonly, aes(x = X2, y = Weight)) + geom_point()+ geom_smooth()+
  geom_text(aes(label = Year))



#x2 versus growth 2 psu
ggplot(Meangrowthsum_turbonly, aes(x = X2, y = Weight2)) + geom_point()+ geom_smooth(method = "lm")+
  geom_text(aes(label = Year))
