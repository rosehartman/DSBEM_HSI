#predicted growth from the smelt cages - Constant temperature, turbidity, or food
...
#one version where temperatures change, one where they don't. 
obs.temp.dat <- select(ungroup(CDEC_wide), Temp.BDL, Temp.RVB)
obs.temp.dat_constanttemp <- select(ungroup(CDEC_wide), Temp.BDL, Temp.RVB) %>%
  mutate(Temp.BDL =20, Temp.RVB = 20)


obs.turb.dat <- select(ungroup(CDEC_wide), NTU.BDL, NTU.RVB)

obs.turb.dat_constant <- select(ungroup(CDEC_wide), NTU.BDL, NTU.RVB)%>%
  mutate(NTU.BDL =15, NTU.RVB = 15)
...
for (s in 1:nrow(beta_hat)) {
  ...
  for (i in 1:n.strata) { 
    ...
    NTU.fx[1,i] <- min1+(1-min1)/(1+exp(-(b.turb*(as.numeric(obs.turb.dat_constant[1,i])-a.turb)))) # Turbidity-consumption
  ...
  for (i in 1:n.strata) { # region = Montezuma, Rio Vista
    for (d in 2:n.days) { # day
      ...
      NTU.fx[d,i] <- min1+(1-min1)/(1+exp(-(b.turb*(as.numeric(obs.turb.dat_constant[d,i])-a.turb)))) # Turbidity-consumption
...
write.csv(all.Llong, "outputs/smeltcagelengths_constantturb.csv")
