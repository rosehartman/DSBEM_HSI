#Ship channel test
#put the ship channel data through my code and see if results are similar to Brock's.
...
n.strata <- 4 # "CM51" "CM54" "CM62" "CM66" "CM74" "CM84"
ex.strata = sort(c(  "CM51", "CM66" ,"CM74", "CM84"))
n.prey <- 13 #may adjust this later
n.days<-115 # summer and fall
...
K[13,] <- c(NA,250,2.4,0.375,0.25) #sino
for (i in 1:5) {
  K[,i] <- K[,i]
}
...
T.M <- c(20,20,21.6,23,21.6) # smith and nobriga had this at 21.6, but I'm putting it at 23 to match brock
...
e.d = c(2226, rep(3180, 12)) #updated energy densities based on Brock's analysis.
#e.d = c(1,823, rep(2590, 11)) #go back to what is in Rose et al. 
...
#Temp <- readRDS("C:/Users/rhartman/OneDrive - California Department of Water Resources/synthesis coodination group/sdwsc/SDWSC/tempmodelpredictions_allsites19952024.rds")
#Turb <- readRDS("C:/Users/rhartman/OneDrive - California Department of Water Resources/synthesis coodination group/sdwsc/SDWSC/turb_dailymeans.rds")
Temp = read_csv("C:/Users/rhartman/OneDrive - California Department of Water Resources/synthesis coodination group/sdwsc/SDWSC/data/Temperature_Data_2023_Imputed.csv")
Turb= read_csv("C:/Users/rhartman/OneDrive - California Department of Water Resources/synthesis coodination group/sdwsc/SDWSC/data/Turbidity_Data_2023_Imputed.csv")


Turbwidesc = Turb %>%
  mutate(Year = year(Date), DOY = yday(Date)) %>%
  rename(Region = Station) %>%
  select(Region, Date, Year, DOY, Turbidity)%>%
  filter(Year == 2023) %>%
  distinct() %>%
  arrange(Region) %>%
  pivot_wider(names_from = Region, values_from = Turbidity, values_fn = mean) 


ggplot(Turbwidesc)+
  geom_line(aes(x = DOY, y = CM51))+
  geom_line(aes(x = DOY, y = CM66))+
  geom_line(aes(x = DOY, y = CM74))+
  geom_line(aes(x = DOY, y = CM84))

Turbwidefsc = filter(Turbwidesc, DOY %in% c(153:268))

test = Turbwidefsc  %>%
  split(list(Turbwidefsc$Year))

Turbsc = array(unlist(test),dim=c(115,7, 1), 
             dimnames = list(c(153:267), names(Turbwidefsc),
                             unique(Turbwidefsc$Year)))

#OK! Now i just need to get rid of the id columns
Turbxsc = Turbsc[c(1:153), c(4:7), c(1)]
Turbx2sc = apply(Turbxsc, 2, as.numeric)

#and same with temperature
Tempwidesc = Temp %>%
  mutate(Year = year(Date), DOY = yday(Date)) %>%
  rename(Region = Station) %>%
  select(Region, Date, Year, DOY,Temp) %>%
  distinct() %>%
  arrange(Region) %>%
  pivot_wider(names_from = Region, values_from = Temp, values_fn = mean)
...
Tempwidefsc = filter(Tempwidesc, DOY %in% c(153:268), Year >2009)

test = Tempwidefsc  %>%
  split(list(Tempwidefsc$Year))

Tempsc = array(unlist(test),dim=c(115,7, 1), 
               dimnames = list(c(153:267), names(Tempwidefsc),
                               unique(Tempwidefsc$Year)))

#OK! Now i just need to get rid of the id columns
Tempxsc = Tempsc[c(1:115), c(4:7), c(1)]
Tempx2sc = apply(Tempxsc, c(2), as.numeric)

#zooplankton
load("C:/Users/rhartman/OneDrive - California Department of Water Resources/synthesis coodination group/sdwsc/SDWSC/ZoopsforObjective1.RData")
...
daystaxa = merge(data.frame(Date = alldays), data.frame(IBMR = unique(zoop$IBMR))) %>%
  merge(data.frame(Region = unique(zoop$Site))) %>%
...
zoopsallsc= zoop %>%
  ...
  pivot_wider(id_cols = c(Region,  Date, Month, doy,  Year),
              names_from = IBMR, values_from = BPUE, values_fn = mean) %>%
  select(Region,  Date, Month, doy,  Year,
         limno, othcaladjuv, pdiapjuv, othcalad, acartela, othclad, allcopnaup, 
         daphnia, othcyc, other, eurytem, pdiapfor, sino) %>%
  rename(Day = doy) 


zoopsmwidefsc = filter(zoopsallsc, Day %in% c(153:305))%>%
  arrange(Region) %>%
  filter(Year == 2023, Region %in% c("CM51", "CM66", "CM74", "CM84"))


test = zoopsmwidefsc %>%
  split(list(zoopsmwidefsc$Region))


zoopsc = array(unlist(test),dim=c(153,18,4, 1), 
             dimnames = list(c(153:305), names(zoopsmwidefsc), unique(zoopsmwidefsc$Region),
                             unique(zoopsmwidefsc$Year)))


#OK! Now i just need to get rid of the id columns
zoopxsc = zoopsc[c(1:153), c(6:18), c(1:4), 1]
zoopx2sc = apply(zoopxsc, c(2,3), as.numeric)
PD.mn.array = zoopx2sc #days by prey by strata by year!


obs.temp.dat <- Tempx2sc
obs.turb.dat <- Turbx2sc
...
for (s in 1:200) {
  ...
  for (h in 1:n.yrs) { # year
    ...
    for (i in 1:n.strata) { # region = Conf, NESuisun, LSac, Sac-Cache, SJR
      Food[1,i,h,] <- PD.mn.array[1,,ex.strata[i]]*t(V[,4]/K[,4]) #this is selecting V and k for the fourth life stage
      ...
      NTU.fx[1,i,h] <- min1+(1-min1)/(1+exp(-(b.turb*(as.numeric(obs.turb.dat[1,i])-a.turb)))) # Turbidity-consumption
      ...
      R <- a.r[4]*(Wt[1,i,h]^b.r[4])*exp(R.Q[stage]*as.numeric(obs.temp.dat[1,i]))
      Feg <- F.a[4]*C[1,i,h]
      U <- U.a[4]*(C[1,i,h]-Feg)
  ...
  for (i in 1:n.strata) { # region = Conf, NESuisun, LSac
    ...
    print(ex.strata[i])
    for (h in 1:n.yrs) { # year
      for (d in 1:n.days) { # day
        Food[d,i,h,] <- PD.mn.array[d,,ex.strata[i]]*t(V[,4]/K[,4])
        ...
        NTU.fx[d,i,h] <- min1+(1-min1)/(1+exp(-(b.turb*(as.numeric(obs.turb.dat[d,i])-a.turb)))) # Turbidity-consumption
        ...
        #respiration
        R <- a.r[4]*(Wt[d,i,h]^b.r[4])*exp(R.Q[stage]*as.numeric(obs.temp.dat[d,i]))
        #egestion
        Feg <- F.a[4]*C[d,i,h]
        #excretion
        U <- U.a[4]*(C[d,i,h]-Feg)
...
#  all.delta.BEM[,,,s] <- delta.BEM
 # all.delta.VB[,,s] <- delta.VB
  all.L <- bind_rows(all.L, data.frame(Day = c(152:267), 
                                       `CM51` = L[,1,], 
                                       `CM66` = L[,2,],
                                       `CM74` = L[,3,],
                                       ...
                                       Year = h, s = s))
  all.VBL[,,s] <- VB.L
  all.Wt <- bind_rows(all.Wt, data.frame(Day = c(152:267), 
                                         `CM51` = Wt[,1,], 
                                         `CM66` = Wt[,2,],
                                         `CM74` = Wt[,3,],
...
all.Wt2 = all.Wt %>%
  pivot_longer(cols = c(CM51:CM84), names_to = "Region", values_to = "Weight") %>%
  filter(!is.na(Weight)) %>%
...
ggplot(Wt.summary, aes(x = Day, y = MWeight)) +
  geom_smooth()+
  facet_wrap(~Region)+
...
#calculate grams per gram per day and average by month and location
Wt.summary = Wt.summary %>%
  group_by( Region) %>%
  mutate(Lagweight = lag(MWeight), Diff = MWeight-Lagweight, Growth = Diff/MWeight) %>%
  ungroup()

Growthsum = Wt.summary %>%
  mutate(Month = case_when(Day %in% c(153:182) ~ 6,
                           Day %in% c(183:213) ~ 7,
                           Day %in% c(214:244) ~ 8,
                           Day %in% c(245:274) ~ 9,
                           Day %in% c(275:305) ~ 10)) %>%
...
ggplot(Growthsum, aes(x = Month, y = Growth)) + 
...
Wtsum2 = Wt.summary %>%
  mutate(Month = case_when(Day %in% c(153:182) ~ 6,
                           ...
                           Day %in% c(245:274) ~ 9,
                           Day %in% c(275:305) ~ 10)) %>%
  group_by(Year, Month, Region2) %>%
   summarize(startweight = first(MWeight), endwieght = last(MWeight), diffweight = endwieght-startweight)



ggplot(Wtsum2, aes(x = Region2, y = diffweight, fill = as.factor(Month))) + geom_col(position = "dodge")+
  facet_wrap(~Year, scales = "free_x")

#now calculate grams per gram per day


Wtsal = all.Wt2 %>%
  select(-Region) %>%
  mutate(Year = case_match(YearX,".2" ~ 2011, ".3" ~2012, ".4" ~ 2013, ".5" ~ 2014, ".6" ~ 2015,
                           ".7" ~ 2016, ".8" ~ 2017, ".9" ~ 2018, "10" ~ 2019, "11" ~ 2020, "12" ~2021))%>%
  group_by(s, Region2, Year) %>%
  arrange(Day) %>%
